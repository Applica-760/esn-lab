...
from projects.utils.eval.filter import filter_by_fold, filter_by_group
...
def compute_and_save_confusion_matrices(
    param_dirs, sample_groups, param_judgment_results, output_dir, class_names, class_order, mode
):
    """
    第2段階: 混同行列の計算とプロット
    """
    ...
    for param_dir in param_dirs:
        ...
        for group in sample_groups:
            ...
            for fold_idx in fold_indices:
                ...
                save_confusion_matrix(
                    cm,
                    class_names,
                    f"{param_name} / {group} / fold {fold_idx} ({mode})",
                    output_path,
                    class_order,
                )
            ...
            save_confusion_matrix(
                cm,
                class_names,
                f"{param_name} / {group} / accumulated ({mode})",
                output_path,
                class_order,
            )
        ...
        save_confusion_matrix(
            total_cm, class_names, f"{param_name} / total ({mode})", output_path, class_order
        )
...
def main(cfg):
    ...
    for mode in modes:
        ...
        # 判定結果の読み込み
        param_judgment_results = load_judgment_results_for_params(param_dirs, mode, judge_dir)
