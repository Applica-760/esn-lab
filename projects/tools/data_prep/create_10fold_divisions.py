...
def build_balanced_10fold(df: pd.DataFrame, seeds_base: int) -> Dict[str, pd.DataFrame]:
    ...
    for i, fold_id in enumerate(suffixes):
        ...
        balanced = (
            pd.concat(parts, axis=0).sample(frac=1.0, random_state=seed_i).reset_index(drop=True)
        )
...
def divide_fold_into_10(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    class_samples = {
        cls: df_sorted[df_sorted["behavior"] == cls].reset_index(drop=True) for cls in [0, 1, 2]
    }
...
def save_divisions(
    main_fold_id: str,
    divisions: Dict[str, pd.DataFrame],
    csv_dir: Path = None,
    npz_dir: Path = None,
