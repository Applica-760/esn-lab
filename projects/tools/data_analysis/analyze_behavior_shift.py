...
# 状態ラベルの定義
STATE_LABELS = {"0": "other", "1": "foraging", "2": "rumination"}
...
def process_and_filter_data(directory, csv_path, output_path):
    ...
    for filename in non_uniform_files:
        ...
        filtered_csv = [csv_series[t] for t in range(9000) if not np.all(npy_data[:, t] == 255)]
...
def analyze_transitions(df):
    ...
    pattern_summary = pd.DataFrame(
        [
            {"pattern": pattern, "count": count, "percentage": count / len(df) * 100}
...
def create_pattern_table(pattern_summary_df, output_path):
    ...
    for idx, (_, row) in enumerate(top_patterns.iterrows(), 1):
        table_data.append(
            [str(idx), row["Transition Pattern"], str(row["Count"]), row["Percentage"]]
        )
    ...
    table = ax.table(
        cellText=table_data, cellLoc="left", loc="center", colWidths=[0.06, 0.228, 0.106, 0.126]
    )
...
def create_pattern_by_length_table(filtered_df, output_path, min_label_duration=None):
    ...
    for _, row in filtered_df.iterrows():
        ...
        data_with_patterns.append(
            {"pattern": categorize_pattern(pattern), "length": row["series_length"]}
        )
        ...
        for j in range(len(state_labels)):
            if is_duration:
                if matrix_masked[i, j] >= 0:
                    ...
                    normalized = (
                        (duration - min_val) / (max_val - min_val) if max_val > min_val else 0.5
                    )
                ...
                if count > 0:
                    ...
                    ax.text(j, i, "0", ha="center", va="center", color="#666666", fontsize=11)
...
def create_visualizations(filtered_df, output_dir):
    ...
    create_pattern_by_length_table(
        filtered_df, output_dir / "pattern_by_length_table_min1000.png", min_label_duration=1000
    )
    create_pattern_by_length_table(
        filtered_df, output_dir / "pattern_by_length_table_min3000.png", min_label_duration=3000
    )
...
出力ファイル:
  ...
  pattern_by_length_table_min3000.png  - 同上（各ラベル最低3000timesteps以上）
    ...
    parser.add_argument(
        "--csv", type=str, required=True, help="file_path列とconverted_300列を含むCSVファイルのパス"
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="結果を出力するディレクトリパス"
    )
